...
model_path = os.path.join(settings.BASE_DIR, 'models', 'best_nilm_model.pkl')
...
appliance_labels = ['Refrigerator', 'Microwave', 'Kettle']
...
def predict():
    input_data = [
        [8.2, 0.0011, 1.9791, 230.2],
        [4.6, 0.0032, 1.0567, 230], 
        [10.5, 0.125, 2.645, 250]    # Unknown appliance data
    ]
    data = pd.DataFrame(input_data, columns=['current(A)', 'energy(kWh)', 'power(kW)', 'voltage(V)'])
